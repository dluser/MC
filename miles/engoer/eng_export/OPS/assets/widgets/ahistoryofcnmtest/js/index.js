...
~ function() {
		...
		var Pict = function(img, title, text, x, y, z, w, h) {
				...
				this.srcimg.src = img;
				this.img.width = 1100;
				this.img.height = 1100;
				this.srcimg.onload = function() {
						picw=this.srcimg.width;
						pich=this.srcimg.height;
						var imgratio = picw/pich;
						if(imgratio>1){imgratio=pich/picw;}
						if (picw>pich){picw=900; pich=900*imgratio;}
						else if (pich>picw){pich=900; picw=900*imgratio;}
						var ict = this.img.getContext('2d');
						//ict.fillStyle = "rgba(65, 182, 230,.7)"; //blue
						ict.fillStyle = "rgba(255, 210, 3,.9)"; //yellow
						ict.fillRect(75, 75, picw+50, pich+50);
						ict.drawImage(this.srcimg, 100, 100, picw, pich);
						this.loaded = true;
		...
		var objects = [];
		for (var i = 0; i < 75; i++){
		var rando = randomInt(0,5);
		if (i%2 === 0 ){rando = rando * -1;} 
		if(i>0&&i<74){objects.push(
		new Pict('pics/50thscans00'+(i+1)+'.jpg','','',rando,-1,24-(i/3),3,3)
		);}
		if (i === 0){objects.push(
		new Pict(pic2,i+1,"",-1,-1,24-i,3,3)
		);}
		if (i === 74){objects.push(
		new Pict("pics/50thscans0075.jpg",'','',-1,-1,24-(i/3),3,3)
		);}
}
